...
{
    [TestClass]
    public class WebBackupRepositoryTabletEnabledTests : WebBackupRepositoryTestBase
    {
        ...
        [TestMethod]
        public async Task WhenWebBackupTabletEnabledIsEnteredWith_CurrectNumVal()
        {
            SetupContext(new[] { _data1 });

            MockStudySettingService
               .Setup(x => x.GetStringValue(It.Is<string>(s => s == "WebBackupTabletEnabled"), It.IsAny<Guid?>()))
               .ReturnsAsync(_data1.Value);

            var repo = GetRepository();

            var numDays = await repo.GetWebBackupTabletValue();
            var expectedNumDays = int.Parse(_data1.Value);
            Assert.AreEqual(expectedNumDays, numDays);
        }

        [TestMethod]
        public async Task WhenWebBackupTabletEnabledIsEnteredWith_StringVal()
        {
        ...
        [TestMethod]
        public async Task WhenWebBackupTabletEnabledIsEnteredWith_NullVal()
        {
